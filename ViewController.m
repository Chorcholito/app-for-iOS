...
- (void)setupBasedView {
    ...
    MJRefreshNormalHeader *header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(MJRefresh_header)];
    
    // Asignamos el header a la tabla
    self.peripheralView.mj_header = header;

    // Ajustar el contentInset para separar la lista del "Pull down to refresh"
    self.peripheralView.contentInset = UIEdgeInsetsMake(70, 0, 0, 0);
}
